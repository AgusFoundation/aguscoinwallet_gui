...
//
// Aguscoinis free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Aguscoinis distributed in the hope that it will be useful,
...
bool WalletApplication::init() {
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Aguscoinwallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Aguscoinwallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have bytecoind with non-default RPC port, you should terminate it and relaunch Aguscoinallet\n"
      "or\n"
      "Set the Local deamon required port in Aguscoinallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
