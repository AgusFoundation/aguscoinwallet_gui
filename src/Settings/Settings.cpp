...
const char DEFAULT_WALLET_FILE_NAME[] = "aguscoinwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.aguscoin.com:20334";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("aguscoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("aguscoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("aguscoinwallet.desktop");
  ...
  res = autorunSettings.contains("AguscoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("AguscoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.aguscoin.aguscoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Aguscoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("AguscoinWallet", appPath);
    } else {
      autorunSettings.remove("AguscoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\aguscoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:aguscoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\aguscoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\aguscoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("aguscoinwallet.cfg"));
